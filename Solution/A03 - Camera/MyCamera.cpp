...
void Simplex::MyCamera::CalculateProjectionMatrix(void)
{
	...
	{
		m_m4Projection = glm::perspective(glm::radians(m_fFOV), fRatio, m_v2NearFar.x, m_v2NearFar.y);
	}
	else //Orthographic
	{
...
void MyCamera::MoveSideways(float a_fDistance){
	vector3 side = Right(Forward(m_v3Target, m_v3Position));

	m_v3Position += side * a_fDistance;
	m_v3Target += side * a_fDistance;
