...
public:
	...
	*/
	...
	/*
	USAGE: Finds the forward vector relative to the camera's position and rotation
	ARGUMENTS: vector3 v3_target -> target vector, vector3 v3_position -> current camera position
	OUTPUT: noramilized forward vector3
	*/
	vector3 Forward(vector3 v3_target, vector3 v3_position);
	/*
	USAGE: Finds the right vector relative to the camera's position and rotation
	ARGUMENTS: vector3 v3_forward -> the camera's noramilized forward vector
